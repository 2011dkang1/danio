...
function spawnText(me, settings, fact) {
  if(!fact){
    var element = me.element = addText("", me.left, me.top);

  }else{
    var element = me.element = addText("", me.left, 462);
  }
...
function Block(me, content, hidden, fact) {
  // log(fact);
  me.fact = fact;
  me.width = me.height = 8;
  me.used = false;
...
function blockBump(me, character) {
  ...
  if(player.power > 1 && me.contents[0] == Mushroom && !me.contents[1]) me.contents[0] = FireFlower;
  if(me.fact){
    spawnText(me, me.fact, true);
  }
  
  // log("This is also where the magic can happen", me.fact, me.top, me.left);
...
//   var prething = new PreThing( xloc,  yloc, FuncSpawner, spawnText, settings);
...
function blockContentsEmerge(me, fact) { // find
  var out = new Thing(me.contents[0], me.contents[1], me.contents[2]);
  addThing(out, me.left, me.top);
  setMidXObj(out, me, true);
  out.blockparent = me;
  out.animate(out, me);
  // log("This is where the magic happens", me.left, me.right, me.top, me.bottom, fact);
